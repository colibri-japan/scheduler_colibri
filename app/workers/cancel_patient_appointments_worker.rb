class CancelPatientAppointmentsWorker
  ...
  def perform(patient_id)
    ...
    now = Time.current.in_time_zone('Tokyo')
	  ...
	  SalaryLineItem.where('service_date > ?', now).where(patient_id: patient_id).update_all(cancelled: true, total_wage: 0, total_credits: 0, invoiced_total: 0, updated_at: now)
