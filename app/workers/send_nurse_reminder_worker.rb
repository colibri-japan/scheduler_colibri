class SendNurseReminderWorker
  ...
  def perform(nurse_id, custom_email_days, options={})
    ...
    custom_email_days.each do |date|
      ...
      custom_day_unavailabilities = Unavailability.where(planning_id: planning.id, nurse_id: nurse_id, starts_at: date.beginning_of_day..date.end_of_day).order(starts_at: :asc)
      selected_appointments << custom_day_appointments.to_a
      selected_appointments << custom_day_unavailabilities.to_a
    end
