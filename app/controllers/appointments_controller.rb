class AppointmentsController < ApplicationController
  ...
  def index
    ...
    if params[:nurse_id].present? 
      @appointments = Appointment.with_nurse_or_second_nurse_by_id(params[:nurse_id]).not_archived.includes(:patient, :nurse, :second_nurse)
    elsif params[:patient_id].present?
      @appointments = Appointment.where(patient_id: params[:patient_id]).not_archived.includes(:patient, :nurse, :second_nurse)
    elsif params[:team_id].present? 
      @appointments = Appointment.with_nurse_or_second_nurse_by_id(Team.find(params[:team_id]).nurses.pluck(:id)).not_archived.includes(:patient, :nurse, :second_nurse)
    else
      @appointments = @planning.appointments.not_archived.includes(:patient, :nurse, :second_nurse)
    end
