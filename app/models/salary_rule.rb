    ...
    def self.calculate_salaries
        ...
        SalaryRule.not_expired_at(now_in_japan).all.each do |salary_rule|
            ...
            nurses.each do |nurse|
                salary_line_items_from_rule = SalaryLineItem.where(nurse_id: nurse.id, salary_rule_id: salary_rule.id).in_range(start_of_month..end_of_today)
                targeted_services = SalaryLineItem.includes(:appointment).where(nurse_id: nurse.id, salary_rule_id: nil, archived_at: nil, cancelled: false, title: targeted_titles).where(appointments: {edit_requested: false}).from_appointments.in_range(start_of_month..end_of_today)
                
                #further targeting services from date condition
                case salary_rule.date_constraint
                when 1
                    #holidays
                    targeted_services = targeted_services.where('DATE(salary_line_items.service_date) IN (?)', HolidayJp.between(start_of_month, end_of_today).map(&:date)) 
                when 2
                    #sundays
                    targeted_services = targeted_services.where('EXTRACT(dow from salary_line_items.service_date) = 0')
                else
                end
                ...
                when 1
                    ...
                    previous_career_bonus = SalaryLineItem.where(nurse_id: nurse.id, salary_rule_id: previous_career_rule.id).in_range(start_of_month..end_of_today).first
