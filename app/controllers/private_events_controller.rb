class PrivateEventsController < ApplicationController
	...
	def create_activity_for_update
		...
		parameters["previous_nurse_name"] = Nurse.find(parameters["nurse_id"][0]).try(:name) if parameters["nurse_id"].present? && parameters["nurse_id"][0].present?
		parameters["previous_patient_name"] = Patient.find(parameters["patient_id"][0]).try(:name) if parameters["patient_id"].present? && parameters["patient_id"][0].present?
